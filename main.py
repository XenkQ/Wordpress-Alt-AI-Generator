...
if mode == '0':
    if not sql_operations.can_connect_to_database():
        ...
        exit(0)
    elif not sql_operations.can_login_with_credentials():
    ...
    replace_empty_alts = use_empty_alt in YES_VARIANTS
    if replace_empty_alts:
        rows_to_change.extend(sql_operations.get_data(
            sql_operations.GetDataOperations.IMAGE_IDS_LINKS_WITH_EXISTING_EMPTY_ALT_ROW))
elif mode == '1':
...
for i, key in enumerate(alt_results):
    ...
    if mode == '0':
        if replace_empty_alts:
            sql_operations.change_data(key, model_response_text_translated,
                                       sql_operations.ChangeDataOperations.UPDATE_EXISTING_IMAGE_ALT_DATA)

        sql_operations.change_data(key, model_response_text_translated,
                                   sql_operations.ChangeDataOperations.INSERT_ROW_WITH_IMAGE_ALT_DATA)
    elif mode == '1':
